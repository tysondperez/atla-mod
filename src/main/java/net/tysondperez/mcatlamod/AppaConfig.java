...
public class AppaConfig
{
    ...
    static
    {
        ...
        CAMERA_DRIVEN_FLIGHT = configurator.comment(
                        "Is bison flight vertical movement driven by the pitch of the game camera?",
        ...
        THIRD_PERSON_ON_MOUNT = configurator.comment(
                        "When mounting and dismounting a bison, should the camera automatically switch third-person perspectives?")
                .define("third_person_on_mount", true);

        configurator.pop();

        configurator.comment(
                        "The values that define the offset of the camera position when riding a bison in the third person camera.")
                .push("camera_offsets");
    ...
    {
        ATLAMod.LOGGER.info("building arrays!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    ...
    public static void printOffsets(){
        ATLAMod.LOGGER.info("arrays built");
        for (int i = 0; i < 3; i++){
            ATLAMod.LOGGER.info("value at arr["+i+"]: "+CAMERA_OFFSETS[0][i].get());
        }
        for (int i = 0; i < 3; i++){
            ATLAMod.LOGGER.info("value at arr["+i+"]: "+CAMERA_OFFSETS[1][i].get());
        }
    }
}
